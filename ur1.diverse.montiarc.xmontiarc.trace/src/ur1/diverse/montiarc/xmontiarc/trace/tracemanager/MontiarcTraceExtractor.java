...
public class MontiarcTraceExtractor implements ITraceExtractor {

	private montiarcTrace.SpecificTrace traceRoot;

	final private List<List<? extends montiarcTrace.States.Value>> valueTraces = Collections
			.synchronizedList(new ArrayList<>());

	private List<montiarcTrace.States.State> statesTrace;

	final private Map<montiarcTrace.Steps.SpecificStep, Integer> stepToStartingIndex = new HashMap<>();
	final private Map<montiarcTrace.Steps.SpecificStep, Integer> stepToEndingIndex = new HashMap<>();
	...
	private final List<montiarcTrace.States.Value> observedValues = new ArrayList<>();

	public MontiarcTraceExtractor() {
	...
	private ValueWrapper getValueWrapper(montiarcTrace.States.Value value, int valueIndex) {
		List<montiarcTrace.States.State> states = value.getStatesNoOpposite();
		montiarcTrace.States.State firstState = states.get(0);
	...
	private montiarcTrace.States.Value getValueAt(int traceIndex, int stateIndex) {
		montiarcTrace.States.Value result = null;
		if (traceIndex >= 0 && traceIndex < valueTraces.size()) {
			final List<? extends montiarcTrace.States.Value> valueTrace = valueTraces.get(traceIndex);
			final montiarcTrace.States.State state = statesTrace.get(stateIndex);
	...
	public ValueWrapper getValueWrapper(int traceIndex, int stateIndex) {
		montiarcTrace.States.Value value = getValueAt(traceIndex, stateIndex);
		if (value == null) {
			return null;
		}
		List<montiarcTrace.States.State> states = value.getStatesNoOpposite();
	...
	private List<Integer> computeStateComparisonList(List<montiarcTrace.States.Value> values) {
		final List<Integer> valueIndexes = new ArrayList<>();
		for (int i = 0; i < values.size(); i++) {
			final montiarcTrace.States.Value value = values.get(i);
			int idx = -1;
	...
	private void updateEquivalenceClasses(montiarcTrace.States.State state) {
		final List<montiarcTrace.States.Value> values = getAllStateValues(state, true);
	...
	private void updateEquivalenceClasses(List<montiarcTrace.States.State> states) {
	...
	public boolean compareStates(EObject eObject1, EObject eObject2, boolean respectIgnored) {
		final montiarcTrace.States.State state1;
		final montiarcTrace.States.State state2;

		if (eObject1 instanceof montiarcTrace.States.State) {
			state1 = (montiarcTrace.States.State) eObject1;
		} else {
			return false;
		}

		if (eObject2 instanceof montiarcTrace.States.State) {
			state2 = (montiarcTrace.States.State) eObject2;
		} else {
			return false;
		}

		final List<montiarcTrace.States.Value> values1 = getAllStateValues(state1);
		final List<montiarcTrace.States.Value> values2 = getAllStateValues(state2);
		...
		for (int i = 0; i < values1.size(); i++) {
			if (!respectIgnored || !isValueTraceIgnored(i)) {
				final montiarcTrace.States.Value value1 = values1.get(i);
				final montiarcTrace.States.Value value2 = values2.get(i);
				if (value1 != value2) {
	...
	public boolean compareSteps(EObject eObject1, EObject eObject2) {
		final montiarcTrace.Steps.SpecificStep step1;
		final montiarcTrace.Steps.SpecificStep step2;

		if (eObject1 instanceof montiarcTrace.Steps.SpecificStep) {
			step1 = (montiarcTrace.Steps.SpecificStep) eObject1;
		} else {
			return false;
		}

		if (eObject2 instanceof montiarcTrace.Steps.SpecificStep) {
			step2 = (montiarcTrace.Steps.SpecificStep) eObject2;
		} else {
			return false;
		}
	...
	public boolean compareStatesWithSteps(EObject eObject1, EObject eObject2, boolean respectIgnored) {
		...
		if (compareStates(state1, state2, respectIgnored)) {
			final List<montiarcTrace.Steps.SpecificStep> endedSteps1 = state1.getEndedSteps();
			final List<montiarcTrace.Steps.SpecificStep> startedSteps1 = state1.getStartedSteps();
			final List<montiarcTrace.Steps.SpecificStep> endedSteps2 = state2.getEndedSteps();
			final List<montiarcTrace.Steps.SpecificStep> startedSteps2 = state2.getStartedSteps();
			if (endedSteps1.size() == endedSteps2.size() && startedSteps1.size() == startedSteps2.size()) {
				boolean result = true;
				for (int i = 0; i < endedSteps1.size(); i++) {
					final montiarcTrace.Steps.SpecificStep endedStep1 = endedSteps1.get(i);
					final montiarcTrace.Steps.SpecificStep endedStep2 = endedSteps2.get(i);
				...
				for (int i = 0; i < startedSteps1.size(); i++) {
					final montiarcTrace.Steps.SpecificStep startedStep1 = startedSteps1.get(i);
					final montiarcTrace.Steps.SpecificStep startedStep2 = startedSteps2.get(i);
	...
	public boolean compareTraces(EObject eObject1, EObject eObject2, boolean respectIgnored) {
		final montiarcTrace.SpecificTrace trace1;
		final montiarcTrace.SpecificTrace trace2;

		if (eObject1 instanceof montiarcTrace.SpecificTrace) {
			trace1 = (montiarcTrace.SpecificTrace) eObject1;
		} else {
			return false;
		}

		if (eObject2 instanceof montiarcTrace.SpecificTrace) {
			trace2 = (montiarcTrace.SpecificTrace) eObject2;
		} else {
			return false;
		}

		final List<montiarcTrace.States.State> states1 = trace1.getStatesTrace();
		final List<montiarcTrace.States.State> states2 = trace2.getStatesTrace();
		...
		for (int i = 0; i < states1.size(); i++) {
			final montiarcTrace.States.State state1 = states1.get(i);
			final montiarcTrace.States.State state2 = states2.get(i);
	...
	private List<montiarcTrace.States.Value> getAllStateValues(montiarcTrace.States.State state) {
		return getAllStateValues(state, false);
	}

	private List<montiarcTrace.States.Value> getAllStateValues(montiarcTrace.States.State state,
			boolean includeHiddenValues) {
		final List<montiarcTrace.States.Value> result = new ArrayList<>();
		for (int i = 0; i < valueTraces.size(); i++) {
			if (includeHiddenValues || !isValueTraceIgnored(i)) {
				final List<? extends montiarcTrace.States.Value> trace = valueTraces.get(i);
				boolean notFound = true;
				for (montiarcTrace.States.Value value : trace) {
					if (value.getStatesNoOpposite().contains(state)) {
						result.add(value);
						notFound = false;
						break;
	...
	private boolean isStateBreakable(montiarcTrace.States.State state) {
		final boolean b = state.getStartedSteps().size() == 1;
		if (b) {
			montiarcTrace.Steps.SpecificStep s = state.getStartedSteps().get(0);
			return !(s instanceof montiarcTrace.Steps.Montiarc_ComponentType_Compute_ImplicitStep);
		}
		return true;
	}
	...
	public StateWrapper getStateWrapper(EObject state) {
		if (state instanceof montiarcTrace.States.State) {
			final int idx = statesTrace.indexOf(state);
			if (idx != -1) {
				final montiarcTrace.States.State state_cast = (montiarcTrace.States.State) state;
	...
	public List<StateWrapper> getStateWrappers(int start, int end) {
		...
		for (int i = startStateIndex; i < endStateIndex + 1; i++) {
			final montiarcTrace.States.State state = statesTrace.get(i);
	...
	public List<ValueWrapper> getValueWrappers(int valueTraceIndex, int start, int end) {
		...
		if (valueTraceIndex < valueTraces.size()) {
			final List<? extends montiarcTrace.States.Value> valueTrace = valueTraces.get(valueTraceIndex);
			for (montiarcTrace.States.Value value : valueTrace) {
	...
	public StepWrapper getStepWrapper(Step step) {
		if (step instanceof montiarcTrace.Steps.SpecificStep) {
			final montiarcTrace.Steps.SpecificStep step_cast = (montiarcTrace.Steps.SpecificStep) step;
			...
			if (step_cast instanceof SequentialStep<?>) {
				subSteps.addAll(((SequentialStep<montiarcTrace.Steps.SpecificStep>) step_cast).getSubSteps());
			}
	...
	public int getValuesTraceLength(int traceIndex) {
		if (traceIndex > -1 && traceIndex < valueTraces.size()) {
			List<? extends montiarcTrace.States.Value> trace = valueTraces.get(traceIndex);
			return trace.size();
		}
		return -1;
	}
	...
	public void statesAdded(List<EObject> states) {
		updateEquivalenceClasses(states.stream().map(e -> (montiarcTrace.States.State) e).collect(Collectors.toList()));
		notifyListeners();
	}
